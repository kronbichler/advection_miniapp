...
namespace DGAdvection
{
  ...
  const unsigned int batched_solver_iterations = 2;
      ...
      for (unsigned int q = 0; q < dofs_per_component; ++q)
        {
          ...
          VectorizedArray<Number> speed_gradu = speed[0] * gradients[0][q];
          for (unsigned int d = 1; d < dim; ++d)
            speed_gradu += speed[d] * gradients[d][q];
          const VectorizedArray<Number> flux =
            (factor_skew * time_factor) * speed_gradu;
          const VectorizedArray<Number> result =
            (-1.0 + factor_skew) * time_factor * u * JxW;
          for (unsigned int d = 0; d < dim; ++d)
            gradients[d][q] = result * speed[d];

          // mass matrix part
          dst_ptr[q] += (flux + u * VectorizedArray<Number>(inv_dt)) * JxW;
        }
  ...
  private:
    ...
    mutable dealii::ndarray<VectorizedArray<Number>, Utilities::pow(n, dim)>
      data_array;
  ...
  public:
    ...
    void
    compute_matrix(SparseMatrix<Number> &sparse_matrix) const
    {
    ...
    const LinearAlgebra::distributed::Vector<Number> &src) const
  {
    ...
    CellwisePreconditionerFDM<dim, fe_degree, Number> precondition;
    ...
    for (unsigned int cell = 0; cell < data.n_cell_batches(); ++cell)
      {
        ...
        if (do_batched_solver)
          {
            CellwiseOperator<dim, fe_degree, Number> local_operator(
      ...
      for (unsigned int stage = first_implicit_stage; stage < n_stages; ++stage)
        {
          ...
          MyVectorMemory<LinearAlgebra::distributed::Vector<Number>> memory;
  ...
  AdvectionProblem<dim>::run(const unsigned int n_global_refinements)
  {
    ...
    DiagonallyImplicitRungeKuttaIntegrator<dim, Number> time_integrator(
